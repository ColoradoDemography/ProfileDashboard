...
```{r include=FALSE, warning=FALSE, message=FALSE, echo=FALSE}
    ...
    if("stats" %in% params$outChk) {
      ...
      tags$h2("Basic Statistics")
      statsTable1(params$fipslist,2010,2016)
      cp_countymap(substr(params$fipslist,3,5)) 
    }
```    
...
```{r include=FALSE, warning=FALSE, message=FALSE, echo=FALSE}     
    ...
    if("pop" %in% params$outChk){
      ...
      tags$h2("Population, Migration and Natural Increase Trends")
     
      srcFix(popTable(substr(params$fipslist,3,5),params$unit,1990,2016))
      county_timeseries(fips=as.numeric(substr(params$fipslist,3,5)),endYear=2016,base=10)
      ms_popage(fips=as.numeric(substr(params$fipslist,3,5)),base=10)
      cocPlot(fips=as.numeric(substr(params$fipslist,3,5)),cName=params$unit,2016)  
    }
...
```{r include=FALSE, warning=FALSE, message=FALSE, echo=FALSE}
    # Population Chatacteristics 
     if("popc" %in% params$outChk){
       ...
       tags$h2("Population Characteristics: Income, Education and Race")
       ms_income(fips=substr(params$fipslist,3,5),base=7.5)
       ms_ed(fips=substr(params$fipslist,3,5),base=10)
       raceTab1(fips=substr(params$fipslist,3,5),ctyname=params$unit,"acs1115") 
       srcFix(raceTab2(fips=substr(params$fipslist,3,5),ctyname=params$unit,"acs1115"))
     }

...
#Start Functions
#1) Utility Functions and conversions

#converting params$fipslist to numeric
fipsN <- as.numeric(substr(params$fipslist,3,5))
fips3 <- substr(params$fipslist,3,5)
...
statsTable1 <- function(cty,sYr,eYr){
  ...
  outTab[6,2] <- paste0("Median House Value (Colorado: ","$",format(as.numeric(MedHHValue_state$b25077001),nsmall=0,
                                              big.mark=","),")+")
  
  outTab[7,1] <- "Sources: *State Demography Office"
  outTab[7,2] <- "+ U.S. Census Bureau, 2011-2015 American Community Survey"
  
  return(kable(outTab, caption="Basic Statistics",align="c",
...
popTable <- function(cty,ctyname,sYr,eYr) { 
 ...
 names(tblHead) <- c(" ", ctyname,state)
  ...
  return(kable(m.OutTab, col.names = names_spaced ,
...
raceTab1 <- function(fips,ctyname,ACS) {
  ...
  return(kable(m.race, col.names = names_spaced,
            caption="Race Trend", row.names=FALSE, #align='lrrr',
           format="latex", booktabs=FALSE)  %>%
           kable_styling(latex_options="hold_position") %>%
           add_header_above(header=tblHead) %>%
...
raceTab2 <- function(fips,ctyname,ACS) {
  ...
  return(kable(m.race, col.names = names_spaced,
            caption="Race Trend", row.names=FALSE, #align='lrr',
            format="latex", booktabs=FALSE)  %>%
           kable_styling(latex_options = "hold_position") %>%
           add_footnote(c("Source: ACS 2015 5-Year File"))
  )
}
...
```{r warning=FALSE, message=FALSE, echo=FALSE, results="asis", eval=outStats}     
    #stats; Basic Statistics
      "Basic Statistics"
      statsTable1(params$fipslist,2010,2016)
      cp_countymap(fips3) 
```    


```{r  message=FALSE, echo=FALSE, result="asis", eval=outPop}     
    #pop: Population, Migration and Natural Increase
    
      "Population, Migration and Natural Increase Trends"
      popTable(fips3,params$unit,1990,2016)
      county_timeseries(fips=fipsN,endYear=2016,base=10)
      ms_popage(fips=fipsN,base=10)
      cocPlot(fips=fipsN,cName=params$unit,2016)  
```


```{r  message=FALSE, echo=FALSE, results="asis", eval=outPopC}
    # Population Chatacteristics 
       "Population Characteristics: Income, Education and Race"
       ms_income(fips=fips3,base=7.5)
       ms_ed(fips=fips3,base=10)
       raceTab1(fips=substr(params$fipslist,3,5),ctyname=params$unit,"acs1115") 
       raceTab2(fips=substr(params$fipslist,3,5),ctyname=params$unit,"acs1115")
```